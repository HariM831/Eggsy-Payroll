...
- **Employees, Calendar, and Settings are PIN-locked.** Enrolling workers,
  correcting attendance history, and pairing/sync configuration all require
  the device PIN (`src/lib/pin.ts`) — set once on first launch.
- **Settings is not on the tab bar.** The bottom nav is Punch · Employees ·
  Calendar. To reach Settings, long-press (~800ms) the Punch screen's header
  ("Punch attendance") — the PIN gate still applies. An unpaired phone also
  shows a small "Pair this phone" line on the Punch screen so a fresh
  install doesn't depend on knowing the long-press.
...
Pairing is an 8-character code typed on the phone — no long token pasted in.
One pairing covers both Wages workers and payroll employees:

1. In the Amino Farms web app, go to **Payroll > Wages > Devices**, tap
   "New device", name it (e.g. "Gate phone"), and copy the 8-character
   pairing code shown.
2. On the phone, long-press the Punch screen's header to open **Settings**
   (PIN-gated), enter the code (`XXXX-XXXX`), and tap **Pair device**.
3. If the office approves it immediately you're paired. Otherwise the phone
   shows a waiting screen and polls every 10s until it's approved, rejected,
   or the code expires — killing the app while waiting is safe, the request
   survives and polling resumes on the next open.
4. Pairing a **replacement** phone (`mode: "replace"`) takes over an
   existing device identity, and the first sync restores that device's
   workers and attendance history — a phone that suddenly fills up is
   expected, not a bug.

The device token itself is never shown again after pairing; Settings shows
only the device name and an **Unpair** button.

### Token storage & revocation

The device token is stored in the IndexedDB `meta` store (`sync-config`, see
`src/lib/sync.ts`), the same place it has always lived — this project does
not use Capacitor secure storage, and no custom encryption scheme was
invented. The token is a **secret sent on every request**, so it is stored
and transmitted as-is (never hashed — a hash cannot authenticate). The PIN
is the opposite case and is already correct in `src/lib/pin.ts`: salt +
hash, verified locally, never sent.

If the server revokes the device (`HTTP 401 { "code": "device_revoked" }`),
the app clears the **token only** and drops to the pairing state. Because
this phone is the source of truth for wage workers, punches and day
overrides, those stores are **never** cleared on revoke — anything not yet
synced exists nowhere else, so clearing them would be permanent data loss.
Re-pair and the pending data pushes normally.
...
node scripts/patch-android-signing.mjs    # pins the committed debug keystore
node scripts/patch-android-version.mjs    # sets versionCode/versionName (see "Versioning" below)
...
how the reference Amino Farms Android build works. The three
`patch-android-*.mjs` scripts re-apply camera permission, the signing key,
and the version numbers each time it's regenerated, since a fresh `cap add`
wipes any manual edits. (The `npm run cap:add:android` script runs all of
this in one step.)

## Versioning & update detection

`cap add android` scaffolds `versionCode 1` / `versionName "1.0"` every
time, so without a patch every shared build would report the same version
and update detection couldn't work. `scripts/patch-android-version.mjs`
fixes that after each `cap add`:

- `versionName` comes from `package.json` `version` (bump it per release).
- `versionCode` is a monotonic integer: `$APP_VERSION_CODE` in CI (the
  workflow passes `github.run_number`), defaulting to `1` locally.

The same numbers are exposed to the JS side via Vite `define` constants
(`__APP_VERSION_CODE__` / `__APP_VERSION_NAME__`, see `vite.config.ts`) as
the browser fallback; the packaged app reads them natively via Capacitor's
`App.getInfo()`. The server reports the newest build via the existing
`/api/wages/device-sync/info` response (`latestVersionCode`, `minVersionCode`,
`apkUrl`) — the Punch screen shows a dismissible "Update available" banner
when a newer build exists, and blocks **sync only** (never punching) when the
build is below `minVersionCode`.

**Distribution note (not changed here):** the workflow ends at
`actions/upload-artifact` with 30-day retention, and artifacts require a
GitHub login — so `apkUrl` cannot point at a raw artifact link. Either cut a
GitHub **Release** and point `apkUrl` at its asset URL, or leave the server's
`apkUrl` unset so the banner just says "ask your supervisor" with no link.
The release process itself was left as-is; only the version-patch step was
...
meta:              key "cached-roles" — role names last seen from the server (Wages enrollment suggestions only)
                   ...
                   key "install-id" — random UUID, generated once per install (survives re-pairing)
                   key "pairing-pending" — pendingId + code of an in-flight pairing request
                   key "version-info" — latest/min version + apkUrl from the server, for the update banner
```
